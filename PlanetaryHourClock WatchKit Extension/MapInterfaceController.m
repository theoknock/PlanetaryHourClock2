...
- (void)awakeWithContext:(id)context {
    ...
    dispatch_source_set_event_handler(annotationUpdateTimer, ^{
        ...
        {
            if ([location distanceFromLocation:userLocation] <= (distance / 6.0))
            {
//                NSLog(@"%@%f %@ %f", (current) ? @"CURRENT\t" : @"", [location distanceFromLocation:userLocation], (([location distanceFromLocation:userLocation] >= distance) ? @">=" : @"<"), distance);
